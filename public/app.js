document.addEventListener("DOMContentLoaded", () => {
	...
	async function loadVideos() {
		...
		try {
			...
			if (response.ok && data.success) {
				...
				videoFeed.innerHTML =
					'<p class="term-empty">Failed to load videos.</p>';
			}
		} catch (error) {
			console.error("Error:", error);
			videoFeed.innerHTML =
				'<p class="term-empty">Failed to connect to server.</p>';
		} finally {
	...
	function renderVideos(videos) {
		...
		Object.entries(groups).forEach(([label, items]) => {
			...
			const summarized = items.filter(
				(v) => v.summary && v.summary.length > 0,
			).length;
		...
		try {
			...
			if (data.success && data.reports.length > 0) {
				...
				document.querySelectorAll(".report-delete-btn").forEach((btn) => {
					btn.addEventListener("click", async () => {
						try {
							await fetch(`/api/reports/${btn.dataset.id}`, {
								method: "DELETE",
							});
							loadReports();
						} catch (err) {
				...
				reportList.innerHTML =
					'<p class="term-empty">No saved reports yet.</p>';
			}
		} catch (err) {
	...
	function saveReportAsFile(content, timestamp) {
		const dateStr = timestamp
			? new Date(timestamp).toISOString().slice(0, 10)
			: new Date().toISOString().slice(0, 10);
		const filename = `intel_briefing_${dateStr}.txt`;
		...
		console.log(`[Reports] Saved report as ${filename}`);
	}

	async function displayReport(id) {
		console.log("[Reports] Displaying report", id);
		reportViewer.innerHTML =
			'<div class="report-loading"><div class="spinner"></div><p>Loading report...</p></div>';
		try {
				...
				reportViewer.innerHTML = `
                    <div class="report-display">
                        <div class="report-display-meta">
                            ...
                            <button class="report-save-btn" id="saveReportBtn-${id}">💾 SAVE</button>
                        </div>
                        <div class="report-display-content">${formattedReport}</div>
                    </div>`;
				document
					.getElementById(`saveReportBtn-${id}`)
					.addEventListener("click", () =>
						saveReportAsFile(r.content, r.generated_at),
					);
	...
	function formatReportText(text) {
		// Convert YouTube URLs in text to clickable links
		function linkifyUrls(html) {
			// Match URLs in [brackets] or standalone YouTube URLs
			return html
				.replace(
					/\[\s*(https?:\/\/(?:www\.)?(?:youtube\.com|youtu\.be)\/[^\]\s]+)\s*\]/g,
					'<a href="$1" target="_blank" rel="noopener" class="report-source-link">📎 source</a>',
				)
				.replace(
		...
		return text
			...
			.map((line) => {
				...
				if (trimmed.startsWith("-") || trimmed.startsWith("*")) {
					return `<div class="report-bullet">${linkifyUrls(escapeHtml(trimmed))}</div>`;
				}
				return `<p class="report-paragraph">${linkifyUrls(escapeHtml(trimmed))}</p>`;
			})
			.join("");
	}
	...
	async function checkAuthStatus() {
		try {
			...
			_authConfigured = data.configured;
			isLoggedIn = data.loggedIn;

			if (!data.configured) {
				authStatus.innerHTML =
					'<span class="auth-not-configured">⚠️ YouTube API not configured</span>';
				authActions.innerHTML =
					'<a href="/.env.example" target="_blank" class="secondary-btn">Setup Guide</a>';
            ...
            `;
				...
				authStatus.innerHTML =
					"<span>Login to save playlists to YouTube</span>";
				authActions.innerHTML =
			...
			authStatus.innerHTML =
				'<span class="auth-not-configured">Auth check failed</span>';
		}
	}
	...
	async function loadDailyWorkspace() {
		try {
			...
			currentDailyVideos = Array.isArray(todayPlaylist.data)
				? todayPlaylist.data
				: [];
	...
	function renderWorkspaceEmpty() {
		pendingVideos.innerHTML =
			'<p class="group-empty">No videos yet. Click "Sync Daily Feed" to start!</p>';
	...
	function renderWorkspace(videos) {
		...
		const groups = {
			ready: videos.filter(
				(v) => (v.status || "pending") === "pending" || v.status === "approved",
			),
		...
		if (groups.ready.length > 0 && window.TopicDedup) {
			const allowList = currentFilters?.allow_list || [];
			const { clusters } = window.TopicDedup.clusterByTopic(
				groups.ready,
				allowList,
			);

			// Render: multi-video clusters get cluster cards, solo gets normal cards
			pendingVideos.innerHTML = clusters
				.map((cluster) => {
			...
			pendingVideos.innerHTML = groups.ready
				.map((v) => renderVideoCard(v, "pending"))
				.join("");
		} else {
			pendingVideos.innerHTML =
		...
		document.getElementById("extractedGroup").style.display =
			groups.extracted.length > 0 ? "block" : "none";
		document.getElementById("ignoredGroup").style.display =
			groups.ignored.length > 0 ? "block" : "none";
	...
	function renderClusterCard(cluster) {
		...
		const channels = [
			...new Set(videos.map((v) => v.channelName || "Unknown")),
		];
		const channelList =
		...
		const thumb =
			videos[0].thumbnail ||
			`https://i.ytimg.com/vi/${videos[0].id}/mqdefault.jpg`;

		// Build the expanded picker rows
		const pickerRows = videos
			.map((v, i) => {
	...
	function renderVideoCard(video, groupType) {
		...
		const filterInfo =
			video.filter_reason && video.filter_reason !== "neutral"
		...
		document.querySelectorAll(".ws-edit-block").forEach((btn) => {
			btn.addEventListener("click", async (e) => {
				...
				const newTerm = prompt(
					`Edit block phrase:\n\nCurrent: "${oldTerm}"\nEnter new phrase (or clear to remove block):`,
					oldTerm,
				);
	...
	function extractSmartKeywords(title) {
		...
		const keywords = words
			.map((w) => w.toLowerCase())
			.filter(
				(w) => w.length >= 3 && !DISMISS_STOP_WORDS.has(w) && !/^\d+$/.test(w),
			)
			.map((kw) => {
				let score = kw.length;
				// Proper noun bonus
	...
	function showDismissPopup(triggerBtn, videoId, title) {
		...
		const chips = keywords
			.slice(0, 4)
			.map(
				(kw) => `<button class="dismiss-chip" data-word="${kw}">${kw}</button>`,
			)
			.join("");
		...
		const card =
			triggerBtn.closest(".ws-video-card") ||
			triggerBtn.closest(".shorts-card");
		if (card) {
		...
		popup.querySelectorAll(".dismiss-chip").forEach((chip) => {
			chip.addEventListener("click", (e) => {
				...
				popup.querySelectorAll(".dismiss-chip").forEach((c) => {
		...
		popup
			.querySelector(".dismiss-just-ignore")
		...
		popup
			.querySelector(".dismiss-block-confirm")
			.addEventListener("click", async (e) => {
				e.stopPropagation();
				...
				if (success) {
					showSuccess(
						`🚫 Blocked "${word}" — future syncs will filter this topic`,
					);
				}
		...
		popup
			.querySelector(".dismiss-custom-input")
	...
	async function _addFilterTerm(term, listType) {
		try {
	...
	async function updateVideoStatus(videoId, newStatus, blockedTerm = null) {
		...
		try {
			...
			const response = await fetch(
				`/api/playlist/${currentDailyFile}/video/${videoId}/status`,
				{
					method: "PATCH",
					headers: { "Content-Type": "application/json" },
	...
	document
		.querySelector(".extract-all-btn")
		?.addEventListener("click", async () => {
			const btn = document.querySelector(".extract-all-btn");
			if (btn.disabled) return; // prevent double-click

			console.log(
				"[Extract All] Clicked. currentDailyVideos:",
				currentDailyVideos.length,
			);
			// Include both pending + approved (the merged "Ready to Extract" group), exclude shorts
			const readyVideos = currentDailyVideos.filter(
				(v) =>
					((v.status || "pending") === "pending" || v.status === "approved") &&
					v.is_short !== true,
			);
			...
			if (readyVideos.length === 0) {
				alert(
					'No videos ready to extract. Click "Sync Daily Feed" first to load videos.',
				);
				return;
			}
			...
			console.log(
				"[Extract All] Starting extraction of",
				readyVideos.length,
				"videos...",
			);
			btn.disabled = true;
	...
	async function _bulkUpdateStatus(videoIds, status) {
		if (!currentDailyFile) return;

		try {
			const response = await fetch(
				`/api/playlist/${currentDailyFile}/bulk-status`,
				{
					method: "PATCH",
					headers: { "Content-Type": "application/json" },
	...
	function renderFilters() {
		...
		allowItems.innerHTML =
			currentFilters.allow_list
				.map(
					(term) =>
						`<div class="filter-item">
                <span>${escapeHtml(term)}</span>
		...
		document.querySelectorAll(".filter-remove-btn").forEach((btn) => {
			btn.addEventListener("click", () =>
				removeFilterTerm(btn.dataset.term, btn.dataset.list),
			);
		});
	}
	...
	async function loadChannels() {
		try {
			...
			channelsList.innerHTML =
				'<p class="channels-empty">Error loading channels.</p>';
		}
	}
	...
	function renderChannels() {
		if (channelsData.length === 0) {
			channelsList.innerHTML =
				'<p class="channels-empty">No channels configured. Add one above!</p>';
			return;
		}

		channelsList.innerHTML = channelsData
			.map(
				(ch, index) => `
		...
		document.querySelectorAll(".toggle-shorts-cb").forEach((cb) => {
			cb.addEventListener("change", async () => {
				...
				await saveChannels();
				showSuccess(
					`${channelsData[index].name}: Shorts ${cb.checked ? "enabled" : "disabled"}`,
				);
			});
		});
	...
	function renderPlaylists(playlists) {
		...
		playlistHistory.innerHTML = playlists
			.map((p, _playlistIndex) => {
					...
					chunks.length > 0
						? chunks
								.map((chunk, index) => {
									...
									const label =
										chunks.length > 1 ? `▶️ Part ${index + 1}` : "▶️ Play";
									return `<a href="${playlistUrl}" target="_blank" class="secondary-btn">${label}</a>`;
								})
								.join("")
						: '<span class="playlist-empty-msg">No active videos</span>';

				// Status summary — merge pending + approved into "ready"
				const ready = allPlaylistVideos.filter(
					(v) =>
						(v.status || "pending") === "pending" || v.status === "approved",
				).length;
				const extracted = allPlaylistVideos.filter(
					(v) => v.status === "extracted",
				).length;
				const ignored = allPlaylistVideos.filter(
					(v) => v.status === "ignored",
				).length;
	...
	function attachPlaylistListeners() {
		...
		document.querySelectorAll(".video-delete-btn").forEach((btn) => {
			btn.addEventListener("click", async (e) => {
				...
				try {
					const response = await fetch(
						`/api/playlist/${filename}/video/${videoId}`,
						{ method: "DELETE" },
					);
					const data = await response.json();
					if (data.success) {
						...
						const playlistCard = document.querySelector(
							`.playlist-card[data-filename="${filename}"]`,
						);
						if (playlistCard) {
		...
		document.querySelectorAll(".extract-btn").forEach((btn) => {
			btn.addEventListener("click", async (e) => {
				...
				if (
					!confirm(
						"Extract transcripts for all videos? This may take several minutes.",
					)
				)
					return;
				...
				let statusDiv = playlistCard.querySelector(
				...
				if (!statusDiv) {
					...
					statusDiv.style.cssText =
						"padding: 0.75rem 1rem; background: rgba(59,130,246,0.1); border-left: 3px solid #3b82f6; margin: 0.5rem 1rem; font-size: 0.85rem; color: #60a5fa;";
				...
				try {
					const response = await fetch(
						`/api/playlist/${filename}/extract-transcripts`,
						{ method: "POST" },
					);
					const data = await response.json();
					...
					if (data.success) {
						...
						showSuccess(
							`Extracted ${data.extracted} of ${data.total} transcripts!`,
						);
					} else {
						throw new Error(data.error || "Extraction failed");
					}
			...
			btn.addEventListener("click", async (e) => {
				...
				try {
					...
					if (data.success) {
						...
						showSuccess(
							`Saved to YouTube! ${data.videosAdded}/${data.totalVideos} videos.`,
						);
	...
	generatePlaylistBtn.addEventListener("click", async () => {
		...
		try {
			...
			if (data.success) {
				...
				document.querySelector("#syncNew strong").textContent =
					data.newCount || 0;
				document.querySelector("#syncDeduped strong").textContent =
					data.dbDeduped || 0;
				document.querySelector("#syncFiltered strong").textContent =
					data.filterDropped || 0;
				document.querySelector("#syncTotal strong").textContent =
					data.videoCount || 0;
	...
	if (discoverBtn) {
		discoverBtn.addEventListener("click", async () => {
			...
			try {
				const response = await fetch("/api/playlist/discover", {
					method: "POST",
				});
				const data = await response.json();
				...
				if (data.success) {
					...
					syncFeedback.classList.remove("hidden");
					document.getElementById("syncNew").innerHTML =
						`✨ <strong>${data.addedToDaily || 0}</strong> discovered`;
					document.getElementById("syncDeduped").innerHTML =
						`🔎 <strong>${data.searchesRun || 0}</strong> searches`;
					document.getElementById("syncFiltered").innerHTML =
						`📦 <strong>${data.candidatesFound || 0}</strong> candidates`;
					document.getElementById("syncTotal").innerHTML =
						`✅ <strong>${data.keptAfterFilter || 0}</strong> kept`;
	...
	addChannelBtn.addEventListener("click", async () => {
		...
		if (
			channelsData.some(
				(c) => c.url === url || c.name.toLowerCase() === name.toLowerCase(),
			)
		) {
	...
	// Sort Channels A-Z
	document
		.getElementById("sortChannelsBtn")
		?.addEventListener("click", async () => {
	...
	document.addEventListener(
		...
		async (e) => {
			...
			try {
				const response = await fetch(`/api/playlist/${filename}`, {
					method: "DELETE",
				});
				const data = await response.json();
	...
	settingsGear.addEventListener("click", async () => {
		...
		try {
			...
			if (sData.success) {
				...
				renderSyncTimes();
				document.getElementById("schedulerStatus").textContent = sData.running
	...
	function renderSyncTimes() {
		...
		container.innerHTML =
			schedulerSyncTimes.length === 0
				? '<span style="font-size:0.7rem;color:#8d6e63;">No times set — add at least one</span>'
				: schedulerSyncTimes
						.map(
							(t) =>
								`<span class="sync-time-pill">${formatTime12h(t)} <span class="sync-time-remove" data-time="${t}">×</span></span>`,
						)
						.join("");
		...
		container.querySelectorAll(".sync-time-remove").forEach((btn) => {
			btn.addEventListener("click", () => {
				schedulerSyncTimes = schedulerSyncTimes.filter(
					(t) => t !== btn.dataset.time,
				);
				renderSyncTimes();
			});
		});
		...
		try {
			...
			if (saveText.startsWith("<!DOCTYPE") || saveText.startsWith("<html")) {
				connectionStatus.textContent =
					"❌ Server needs restart! New routes not loaded.";
				connectionStatus.className = "connection-status status-error";
				return;
			}
			...
			if (text.startsWith("<!DOCTYPE") || text.startsWith("<html")) {
				connectionStatus.textContent =
					"❌ Server needs restart! API routes not available.";
				connectionStatus.className = "connection-status status-error";
				return;
			}
			...
			try {
				...
				if (schedData.success) {
					...
					document.getElementById("schedulerStatus").textContent =
						schedData.running
							? `✅ Running — syncs at ${timeStr}`
							: "⏸️ Scheduler is off";
				} else {
					document.getElementById("schedulerStatus").textContent =
						`❌ ${schedData.error || "Failed"}`;
				}
			} catch (schedErr) {
	...
	if (pipelineBtn) {
		pipelineBtn.addEventListener("click", () => {
			...
			const evtSource = new EventSource(
				`/api/pipeline/run/${currentDailyFile}`,
			);
			...
			evtSource.onmessage = (event) => {
				...
				if (d.phase === "extract") {
					...
					if (d.type === "start") {
						...
						currentTitle.textContent =
							d.step === "extracting-facts"
								? `Extracting facts (${d.current}/${d.total})...`
								: d.step === "categorizing"
									? `Categorizing (${d.current}/${d.total})...`
									: d.step === "bottom-line"
										? "Writing bottom line..."
										: d.step;
	...
	document
		.getElementById("modalSummarizeBtn")
		.addEventListener("click", async () => {
	...
	document
		.getElementById("summarizeAllBtn")
		.addEventListener("click", async () => {
			console.log("[Summarize All] Button clicked!");
			console.log("[Summarize All] allVideos count:", allVideos.length);

			const unsummarized = allVideos.filter(
				(v) => !v.summary && v.transcript_length > 50,
			);
			console.log(
				"[Summarize All] Unsummarized videos found:",
				unsummarized.length,
			);
			...
			console.log(
				"[Summarize All] Progress banner element:",
				progressDiv ? "found" : "MISSING!",
			);
			...
			for (let i = 0; i < unsummarized.length; i++) {
				...
				console.log(
					`[Summarize All] Processing ${i + 1}/${unsummarized.length}: "${v.title}" (id: ${v.id})`,
				);

				try {
					const resp = await fetch(`/api/summarize/${v.id}`, {
						method: "POST",
					});
					const data = await resp.json();
					if (data.success) {
						...
						console.log(
							`[Summarize All] ✅ Success: "${v.title}" (${data.summary?.length || 0} chars)`,
						);
						...
						fail++;
						console.log(
							`[Summarize All] ❌ Failed: "${v.title}" — ${data.error || "Unknown error"}`,
						);
					...
					fail++;
					console.error(
						`[Summarize All] ❌ Error: "${v.title}" — ${e.message}`,
					);
			...
			// Done
			console.log(
				`[Summarize All] ✅ Complete! ${success} succeeded, ${fail} failed out of ${unsummarized.length}`,
			);
		...
		.addEventListener("click", async () => {
			...
			try {
				...
				console.log(
					...
					data.success ? "Success" : data.error,
				);
				...
				if (data.success) {
                    ...
                    <div class="report-display">
                        <div class="report-display-meta">
                            ${timestamp} · ${data.videoCount} sources · Report #${reportId}
                            <button class="report-save-btn" id="saveNewReportBtn">💾 SAVE</button>
                        </div>
                        <div class="report-display-content">${formattedReport}</div>
                    </div>`;
					document
						.getElementById("saveNewReportBtn")
						.addEventListener("click", () =>
							saveReportAsFile(data.report, data.generatedAt),
						);

					// Log benchmarks to console
					if (data.benchmarks) {
						const b = data.benchmarks;
						console.log(
							`[Benchmark] Model: ${b.model} | ${b.steps?.length || 0} calls | ${(b.totalTimeMs / 1000).toFixed(1)}s | avg ${b.avgTokPerSec} tok/s`,
						);
					}

					console.log(
						`[News Report] Displayed report (${data.report.length} chars from ${data.videoCount} videos)`,
					);
...
document.addEventListener("DOMContentLoaded", () => {
	...
	if (resetBtn) {
		resetBtn.addEventListener("click", async () => {
			if (
				!confirm(
					"⚠️ This will reset ALL video statuses (approved, ignored, extracted) back to pending.\\n\\nAre you sure?",
				)
			) {
				return;
			}
