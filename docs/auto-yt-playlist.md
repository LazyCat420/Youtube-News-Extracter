...
Build a system to automatically generate daily YouTube playlists from a curated list of channels that the user can add manually themselves without requiring a YouTube account login. The system will:

1. **Fetch** latest videos from a list of user-defined channels via links to the channel page.
2. **Filter** for videos published in the last 24 hours (or since last run). That way it can pick up the videos that I missed from the last time I looked at those channels.
3. **Generate** an anonymous "Untitled Playlist" link (using `watch_videos?video_ids=`) for easy viewing on PC/Obsidian. Also allow smart playlist generation for specific topics ie news/finance/tech should be one, cooking recipes for another, and general watch later for everything else.
